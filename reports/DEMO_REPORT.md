...
```
## 17. Method Discovery Loop: Traceable Compact Compiler Integration

新增 M2/M3/M5 integration slice：
...
它的部署产物不是单独的 `compact_skill.md`，而是：
...
| Variant | Total Tokens | Simple | Semantic | Trace | Gate |
|---|---:|---|---|---|---|
| A plain compact skill | 265 / 237 | fail | fail | fail | not gated |
| B compressed compact skill | 195 / 237 | pass | pass | fail | not gated |
| C compressed + protocol | 300 / 237 | pass | pass | pass | not gated |
| D compressed + protocol + gate | 300 / 237 | pass | pass | pass | reject_over_budget |
...
- trace verifier 能阻止只靠 rule_id 覆盖的 shallow output。
- protocolized variant 通过 trace verifier，但 protocol token overhead 让总成本超出固定预算。
- validation gate 正确拒绝了超预算部署产物。
