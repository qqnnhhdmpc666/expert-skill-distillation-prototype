...
- Use only these capability_id values exposed by the Skill Package: CONFIG_AUDIT_EXPORT.
...
Review each exposed capability_id one by one; for every capability that is concretely grounded in the target, emit a separate finding.
